...
class BlorqueScriptParserTest {
	...
	@Test def void testFieldDeclaration() {
		...
		assertEquals('fields should have types', null, (field0 as BSField).RType);
		assertEquals('fields should have types', BSPrimitiveType.NONE, (field1 as BSField).PType);
	...
	@Test def void testComplexMethodDeclaration() {
		...
		class A {
			void m0() { return; }
			A f;
			A[] m1(string p0, taggedString p1) { f = f; return null; }
		...
		assertEquals('field should have a name', 'aaf', alsoAField.name);
		...
		assertEquals('field should be A type', BSPrimitiveType.NONE, (field as BSField).PType);
		assertEquals('field should be A type', 'A', (field as BSField).RType.name);
		assertEquals('method should be A type', BSPrimitiveType.NONE, (method1 as BSMethod).PType);
		...
		assertEquals('field should be object type', null, (alsoAField as BSField).RType);
		...
		assertEquals('method2 param1 should be a taggedString named p1', BSPrimitiveType.TAGGED_STRING, param1.PType);
		assertEquals('method2 param1 should be a taggedString named p1', 'p1', param1.name);
